...
= Introduction
This project is about building and implementing a _*Cloud-Native*_ architecture for Real-Time weather telemetry using _Apache Kafka_. The system is designed to collect, process, and store Real-Time meteorological data from six different cities in Italy: Milan, Turin, Verona, Florence, Rome and Naples (the data is retrieved via _REST API_ available on #link("https://open-meteo.com/")[`open-meteo.com`]). In addition, a continuous stream of measurements (temperature, rainfall, wind speed...) is simulated by an external script and sent to the Kafka cluster. From there, the data is consumed in parallel by multiple microservices for specific purposes: Real-Time data visualization via a web dashboard, data storage in _NoSQL_ database and alert generation for extreme weather conditions, forwarded to a smartphone via Telegram bot. The main focus is to guarantee the system's scalability, reliability, cryptographic security and fault-tolerance.


= Architectural overview
The entire infrastructure is based on an architectural pattern with decoupled microservices. The communication is not direct, but rather mediated by a message broker, in this case Apache Kafka. This is the architectural overview of the system:
...
The backbone of the architecture is represented by a distributed messaging cluster (it is the "_Single Source of Truth_") for all the microservices:
- *Apache Kafka*: It manages data flow and divides it into *Topics* (`weather.telemetry` and `weather.alerts`). Kafka brokers do not push data to the *Consumers*; rather, they act as high-performance immutable registers from which microservices pull data autonomously and at their own pace.
...
- *Docker*: each microservice is packaged in an isolated container, starting with a specific image (`python:3.11-slim`). This approach guaratees code portability and consistency across different environments.
...
- *Fault Tolerance*: Kafka cluster is configured with a replication factor of 3. Data is replicated across physically separated nodes, so, in case of a node failure, _Zookeeper_ automatically promotes a backup replica (_ISR_) to a new leader within few milliseconds, without any event loss or interruption of the service.
...
- *Load Balancing*: this requirement is handled natively by Apache Kafka architecture, combining topic partitioning with the strategic use of consumer groups. Services like Notifier and Storage use static Group IDs: in case of horizontal scaling, Kafka automatically assigns the topic's partitions among the replicas of the microservice, balancing the computational load evenly. The only exception is the *Dashboard*, which uses a randomly generated Group ID (backed by a sufficiently large keyspace) to ensure each instance is treated as an independent consumer group, allowing all instances to consume the same data in parallel without load balancing, implementing a _broadcasting pattern_ suited for real-time visualization.

= Showcases
#figure(
  image("/img/kafka-UI.png", width: 80%),
  caption: [Load Balancing on 6 partitions of `weather.alerts` topic, viewable thanks to #link("https://github.com/provectus/kafka-ui")[Kafka-UI].]
) <fig:kafka-UI>
